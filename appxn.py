...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.appxp")
pluginrootpath = "plugin://plugin.video.appxp"
...
def GetSheetIDFromSettings():
	sid = "1RelNlIujogGH3ORvlJ_CESRKx9nhp3TSl9GbAVwi1EU"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.appxp/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.appxp(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.appxp" in item["path"]:
